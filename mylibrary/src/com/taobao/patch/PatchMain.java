...
//			String dexPath = new File(context.getFilesDir(), apkPath.).getAbsolutePath();
			...
			while (entrys.hasMoreElements()) {
				...
				try {
					if(!entry.contains("android")){
						Log.i("jw", "entry:"+entry);
					}
					entryClass = mcl.loadClass(entry);
				} catch (Throwable e) {
					if("com.taobao.patch.DialogPatch".equals(entry)){
						Log.i("jw", "error:"+Log.getStackTraceString(e));
					}
					//Log.i("jw", "entry:"+entry+",error:"+Log.getStackTraceString(e));
					continue;
				}
				if (isImplementInterface(entryClass, IPatch.class)) {
					Object moduleInstance = entryClass.newInstance();
					Log.i("jw", "patchobj:"+moduleInstance);
