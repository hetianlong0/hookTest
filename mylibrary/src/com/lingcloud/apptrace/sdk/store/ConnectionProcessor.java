...
public class ConnectionProcessor implements Runnable {
    ...
    ConnectionProcessor(final String serverURL, final lingAgentStore store, final DeviceId deviceId, final SSLContext sslContext) {
    ...
    URLConnection urlConnectionForEventData(final String eventData) throws IOException {
        ...
        if (DclingCloudAgent.isLoggingEnabled()) {
            Log.d(DclingCloudAgent.TAG, "Got picturePath: " + picturePath);
        }
...
//            BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(os));
    ...
    public void run() {
        while (true) {
            ...
            if (deviceId_.getId() == null) {
                ...
                if (DclingCloudAgent.isLoggingEnabled()) {
                    Log.i(DclingCloudAgent.TAG, "No Device ID available yet, skipping request " + storedEvents[0]);
                }
                break;
            }
            ...
            try {
                ...
                if (conn instanceof HttpURLConnection) {
                    ...
                    if (!success && DclingCloudAgent.isLoggingEnabled()) {
                        Log.w(DclingCloudAgent.TAG, "HTTP error response code was " + responseCode + " from submitting event data: " + eventData);
                    }
                }
                ...
                if (success) {
                    ...
                    if (!success && DclingCloudAgent.isLoggingEnabled()) {
                        Log.w(DclingCloudAgent.TAG, "Response from lingcloud server did not report success, it was: " + responseData.toString("UTF-8"));
                    }
                }

                if (success) {
                    if (DclingCloudAgent.isLoggingEnabled()) {
                        Log.d(DclingCloudAgent.TAG, "ok ->" + eventData);
                    }
                ...
                if (DclingCloudAgent.isLoggingEnabled()) {
                    Log.w(DclingCloudAgent.TAG, "Got exception while trying to submit event data: " + eventData, e);
                }
    ...
    lingAgentStore getlingAgentStore() { return store_; }
