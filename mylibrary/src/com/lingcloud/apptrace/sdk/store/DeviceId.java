...
public class DeviceId {
    /**
     * Enum used throughout lingcloud which controls what kind of ID lingcloud should use.
     */
...
//    private static final String PREFERENCE_KEY_ID_TYPE = "ly.count.android.api.DeviceId.type";
     ...
     * In some cases, lingcloud can override ID generation strategy to other one, for example when
     * Google Play Services are not available and user chose Advertising ID strategy, it will fall
     * back to OpenUDID
     * @param context Context to use
     * @param store lingStore to store configuration in
     * @param raiseExceptions whether to raise exceptions in case of illegal state or not
     */
    public void init(Context context, lingAgentStore store, boolean raiseExceptions) {
...
//                    }
//                } else {
//                    // just do nothing, without Advertising ID and OpenUDID this user is lost for lingcloud
//                    if (DclingCloudAgent.isLoggingEnabled()) {
...
//    private void storeOverriddenType(lingAgentStore store, Type type) {
...
//    }
//
//    private Type retrieveOverriddenType(lingAgentStore store) {
//        // Using strings is safer when it comes to extending Enum values list
...
//        }
    ...
    protected void switchToIdType(Type type, Context context, lingAgentStore store) {
        if (DclingCloudAgent.isLoggingEnabled()) {
...
//        storeOverriddenType(store, type);
    ...
    /**
     * Helper method for null safe comparison of current device ID and the one supplied to lingcloud.init
