...
public class UserData {
     ...
     */
    public void save2(){
        DclingCloudAgent.getInstance().commandFactory_.sendUserData();
//        UserData.clear();
    }
    ...
    public static void setData(Map<String, String> data){
        ...
        if(picturePath != null){
            ...
            if (!sourceFile.isFile()) {
                if (DclingCloudAgent.isLoggingEnabled()) {
                    Log.w(DclingCloudAgent.TAG, "Provided file " + picturePath + " can not be opened");
                }
                picturePath = null;
            }
        }
        ...
        if(data.containsKey(BYEAR_KEY)){
            try {
                ...
                if (DclingCloudAgent.isLoggingEnabled()) {
                    Log.w(DclingCloudAgent.TAG, "Incorrect byear number format");
                }
                byear = 0;
            }
        }
...
//                            result += "&" + PICTURE_PATH_KEY + "=" + java.net.URLEncoder.encode(picturePath, "ASCII");
                        }
                    }
                    else{
                        result = "";
...
//                            result += "&user_details&" + PICTURE_PATH_KEY + "=" + java.net.URLEncoder.encode(picturePath, "ASCII");
                        }
                    }
    ...
    static JSONObject toJSON() {
        ...
        try {
            ...
            if (DclingCloudAgent.isLoggingEnabled()) {
                Log.w(DclingCloudAgent.TAG, "Got exception converting an UserData to JSON", e);
            }
        }
        ...
        if(json != null){
            ...
            if (!json.isNull(CUSTOM_KEY)) {
                ...
                try {
                    ...
                    if (DclingCloudAgent.isLoggingEnabled()) {
                        Log.w(DclingCloudAgent.TAG, "Got exception converting an Custom Json to Custom User data", e);
                    }
                }
            }
        }
            ...
            for (String pair : pairs) {
                ...
                if(pair.substring(0, idx).equals(PICTURE_PATH_KEY)){
                    try {
                        ...
                        //htl add;
//                        ret = URLDecoder.decode(pair.substring(idx + 1), "ASCII");
